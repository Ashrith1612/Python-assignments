        ...
        for k in range(1, len(lst_country)):
            if i == lst_country[k] and j == lst_date[k]:
                ...
                if count_of_same_country_same_date == 1:
                    ...
                    lst_aggr_confirmed_cases_by_country[-1] += int(lst_confirmed_cases[k])
                    lst_aggr_recovered_cases_by_country[-1] += int(lst_recovered_cases[k])
                    lst_aggr_deaths_by_country[-1] += int(lst_deaths[k])
    plt.plot(lst_dates_for_aggr_data_by_country, lst_aggr_confirmed_cases_by_country)
    plt.plot(lst_dates_for_aggr_data_by_country, lst_aggr_recovered_cases_by_country)
...
print("The total number of deaths as of the last recorded date is", total_global_deaths)
